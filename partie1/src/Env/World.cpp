    ...
    while (humidity > humidityThreshold_)
        {
            humidity = humidityInitialLevel_
                    * std::exp (
                            (float) -humidityRange_
                                    / (float) humidityDecayRate_);
            ++humidityRange_;
        }
}
...
World::updateCache ()
{
    ...
    auto minmax (
            std::minmax_element (humidityLevels_.begin (),
                                 humidityLevels_.end ()));
...
World::reset (bool regenerate)
{
    ...
    for (size_t i (0); i < seeds_.size (); ++i)
        {
            size_t index (
                    seeds_[i].position.y * numberColumns_
                            + seeds_[i].position.x);
    ...
    for (size_t i (0); i < cells_.size (); ++i)
        {
            ...
            for (size_t dx (scanRange.left);
                    dx <= scanRange.left + scanRange.width; ++dx)
                {
...
{
    return ((position.x / cells_.size ()) * numberColumns_
            + position.y / cells_.size ());
}
