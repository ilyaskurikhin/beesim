...
Bee::Bee (Hive* hive, const Vec2d& position, double radius, double speedAmpl,
          double energy) :
    Collider (position, radius), hive_ (hive), speedAmpl_ (speedAmpl), energy_ (
        energy)
{
  reloadConfig ();
...
Bee::reloadConfig ()
{
  rotationProbability_ =
      (getAppConfig ()["simulation"]["bees"]["generic"]["moving behaviour"]["random"]["rotation probability"].toDouble ());
  maxAngle_ =
      (getAppConfig ()["simulation"]["bees"]["generic"]["moving behaviour"]["random"]["rotation angle max"].toDouble ());
}
